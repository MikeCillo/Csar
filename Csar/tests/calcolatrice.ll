...
declare void @scribe_error() local_unnamed_addr
...
define i32 @calcola(i32 %op, i32 %a, i32 %b) local_unnamed_addr {
entry:
...
common.ret:                                       ; preds = %entry, %ok, %then.4, %then.2, %then.1, %then
  %common.ret.op = phi i32 [ %addtmp, %then ], [ %subtmp, %then.1 ], [ %multmp, %then.2 ], [ 0, %then.4 ], [ %divtmp, %ok ], [ 0, %entry ]
...
then.3:                                           ; preds = %entry
  ...
  br i1 %cmptmp.4, label %then.4, label %ok

then.4:                                           ; preds = %then.3
  tail call void @scribe_error()
  br label %common.ret
...
then.3.i:                                         ; preds = %else.1
  ...
  br i1 %cmptmp.4.i, label %then.4.i, label %ok.i

then.4.i:                                         ; preds = %then.3.i
  tail call void @scribe_error()
  br label %calcola.exit
...
calcola.exit:                                     ; preds = %else.1, %then.i, %then.1.i, %then.2.i, %then.4.i, %ok.i
  %common.ret.op.i = phi i32 [ %addtmp.i, %then.i ], [ %subtmp.i, %then.1.i ], [ %multmp.i, %then.2.i ], [ 0, %then.4.i ], [ %divtmp.i, %ok.i ], [ 0, %else.1 ]
